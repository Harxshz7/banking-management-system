...
public class DataManager {
    ...
    private List<RecurringDeposit> rds = new ArrayList<>();
    ...
    private void seedDefaultData() {
        ...
        List.of(
                new Transaction(johnSavings.getId(), john.getId(), Transaction.TransactionType.ACCOUNT_OPENED, 8500,
                        8500, "Account Opened", null),
                new Transaction(johnChecking.getId(), john.getId(), Transaction.TransactionType.ACCOUNT_OPENED, 3200,
                        3200, "Account Opened", null),
                new Transaction(johnFD.getId(), john.getId(), Transaction.TransactionType.ACCOUNT_OPENED, 25000, 25000,
                        "Fixed Deposit Opened", null),
                new Transaction(janeSavings.getId(), jane.getId(), Transaction.TransactionType.ACCOUNT_OPENED, 12000,
                        12000, "Account Opened", null),
                new Transaction(janeChecking.getId(), jane.getId(), Transaction.TransactionType.ACCOUNT_OPENED, 4500,
                        4500, "Account Opened", null))
    ...
    public List<Notification> getNotificationsByUser(String userId) {
        return notifications.stream().filter(n -> n.getUserId().equals(userId)).collect(Collectors.toList());
    }
    ...
    public void markNotificationRead(String notificationId) {
        notifications.stream().filter(n -> n.getId().equals(notificationId)).findFirst().ifPresent(n -> {
            n.setRead(true);
            saveAll();
        });
    }
    ...
    public List<RecurringDeposit> getAllRds() {
        return new ArrayList<>(rds);
    }

    public List<RecurringDeposit> getRdsByUser(String userId) {
        return rds.stream().filter(d -> d.getUserId().equals(userId)).collect(Collectors.toList());
    }
    ...
    public List<ScheduledTransfer> getScheduledTransfersByUser(String userId) {
        return scheduledTransfers.stream().filter(s -> s.getUserId().equals(userId)).collect(Collectors.toList());
    }
    ...
    private void loadAll() {
        ...
        notifications = loadFile(NOTIFICATIONS_FILE);
        auditLogs = loadFile(AUDIT_FILE);
        rds = loadFile(RDS_FILE);
        scheduledTransfers = loadFile(SCHEDULED_FILE);
    }
    ...
    public void saveAll() {
        ...
        saveFile(AUDIT_FILE, auditLogs);
        saveFile(RDS_FILE, rds);
        saveFile(SCHEDULED_FILE, scheduledTransfers);
    }
